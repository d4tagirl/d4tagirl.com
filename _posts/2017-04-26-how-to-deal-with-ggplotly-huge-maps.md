...
<!--more-->
I've been experimenting with interactive maps and animated ones these past few days and I finally was able to produce some fancy outputs using the `plotly` package and the `gganimate` one. You can see the whole process [in this blog post]({% post_url 2017-05-10-visualizing-r-ladies-growth %}){:target="_blank"} or [in my GitHub repo](https://github.com/d4tagirl/R-Ladies-growth-maps). Everything was working perfectly on Rstudio, until I wanted to share my results on my blog, and _things got complicated_. Originally I produced some animated maps using the `plotly::ggplotly` function and it generated a 3.3 MB HTML widget... and *that's an issue* when you want to include it on a website 😳 . So I tried a different approach: using the `plotly::plot_geo` function I was able to produce a very similar plot, generating a way smaller HTML widget.
